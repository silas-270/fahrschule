...
app.post('/admin/appointments', adminAuth, async (req, res) => {
    try {
        const { user_id, date_time, type } = req.body;

        if (!user_id || !date_time || !type) {
            return res.status(400).json({
                success: false,
                message: 'Benutzer-ID, Datum/Uhrzeit und Typ sind erforderlich'
            });
        }
        ...
        const user = await db.get('SELECT id FROM users WHERE id = $1', [user_id]);
        if (!user) {
            return res.status(404).json({
                success: false,
                message: 'Benutzer nicht gefunden'
        ...
        // Erstelle den Termin
        const result = await db.run(
            `INSERT INTO appointments (user_id, date_time, type, status) 
             VALUES ($1, $2, $3, 'suggested') 
             RETURNING id`,
            [user_id, date_time, type]
        );
...
// Termin löschen
app.delete('/admin/appointments/:id', adminAuth, async (req, res) => {
    try {
        const { id } = req.params;
        const db = await openDb();
        
        const result = await db.run('DELETE FROM appointments WHERE id = $1', [id]);
...
// Alle Termine eines Benutzers abrufen (Admin)
...
app.put('/admin/appointments/:id', adminAuth, async (req, res) => {
    try {
        const { id } = req.params;
        const { date_time, type, status } = req.body;

        if (!date_time && !type && !status) {
            return res.status(400).json({
                success: false,
        ...
        if (status && !['suggested', 'accepted', 'rejected'].includes(status)) {
            return res.status(400).json({
                success: false,
                message: 'Ungültiger Status'
            });
        ...
        const appointment = await db.get('SELECT * FROM appointments WHERE id = $1', [id]);
        if (!appointment) {
            return res.status(404).json({
                success: false,
                message: 'Termin nicht gefunden'
        ...
        // Baue das UPDATE-Statement dynamisch auf
        const updates = [];
        const params = [];
        let paramIndex = 1;
